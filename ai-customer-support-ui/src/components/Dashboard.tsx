...
function Dashboard() {
  ...
  const fetchHistoryFiles = async () => {
    try {
      const res = await fetch('http://localhost:8003/list-file-scores');
  ...
  const generatePDF = async (filename: string) => {
    ...
    try {
      const scoresRes = await fetch(`http://localhost:8003/get-file-scores/${encodeURIComponent(filename)}`);
      ...
      const summaryRes = isAudioFile
        ? await fetch(`http://localhost:8000/get-file-summary/${encodeURIComponent(originalName)}`).catch(() => null)
        : await fetch(`http://localhost:8001/get-file-summary/${encodeURIComponent(originalName)}`).catch(() => null);
      const summaryData = summaryRes?.ok ? await summaryRes.json() : null;

      const transcriptRes = isAudioFile
        ? await fetch(`http://localhost:8000/get-transcript?t=${Date.now()}`).catch(() => null)
        : await fetch(`http://localhost:8001/get-text-transcript?t=${Date.now()}`).catch(() => null);
